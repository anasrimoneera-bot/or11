...
ssh -o StrictHostKeyChecking=accept-new %SERVER% "set -e; cd %REPO% && echo '=== 1/7 备份业务数据库 ===' && (if [ -f data/erp.db ]; then BK=data/erp.db.bak-$(date +%%Y%%m%%d-%%H%%M%%S) && cp data/erp.db $BK && echo \" 已备份到: $BK ($(du -h $BK | awk '{print $1}'))\"; else echo ' (跳过：无数据库文件)'; fi) && echo '' && echo '=== 2/7 检测本地未提交改动 ===' && DIRTY=$(git status --porcelain) && if [ -n \"$DIRTY\" ]; then echo ' 警告：服务器上有未提交改动:'; echo \"$DIRTY\" | head -10; echo ' 已自动 stash 暂存（可用 git stash list 查看，git stash pop 恢复）'; git stash push -u -m \"auto-stash-before-deploy-$(date +%%Y%%m%%d-%%H%%M%%S)\"; else echo ' 无未提交改动'; fi && echo '' && echo '=== 3/7 清理自动生成的 lock 文件（避免冲突）===' && rm -f package-lock.json client/package-lock.json && echo ' OK' && echo '' && echo '=== 4/7 拉取最新代码 ===' && git config http.version HTTP/1.1 && git pull origin %BRANCH% 2>&1 | tail -20 && echo '' && echo '=== 5/7 安装依赖 + 构建前端 (约 30-90 秒) ===' && npm install --no-audit --no-fund --omit=optional 2>&1 | tail -10 && echo '' && echo '=== 6/7 重启 pm2 服务 ===' && pm2 restart %PM2_NAME% --update-env && pm2 ls && echo '' && echo '=== 7/7 启动日志（最后 15 行） ===' && sleep 1 && pm2 logs %PM2_NAME% --lines 15 --nostream && echo '' && echo '=== 部署完成 ===' && echo \"当前 HEAD: $(git log -1 --oneline)\""
